---
postTitle: 'Complete(Docker) Python Back-end Deployment On AWS EC2'
...
postThumbnailUrl: "/blog/post-banners/post-banner_10.png"
...
at Zed Labs."
postTags: ['docker', 'python', 'django', 'aws', 'ec2', 'nginx', 'ssl', 'devops', 'deployment']
...
![post banner](/blog/post-banners/post-banner_10.png)
...
<GitHubEmbed owner="Okpainmo" repo="fast-django-backend-template" />

> N.B: This guide assumes, that you already created an EC2 instance, and is ready for the API server deployment.
> In case you haven't and need a guide to go about that,
> [explore this other article](/blog/categories/cloud-and-devops/how-to-create-an-aws-ec2-instance), in
...
![(a post image sharing more information)](/blog/_10/10_img-3.png)
...
![(a post image sharing more information)](/blog/_10/10_img-4.png)
![(a post image sharing more information)](/blog/_10/10_img-5.png)
...
<CodeBlock language="bash" content={`cd aws-ec2-django-deployment-project`} />

![(a post image sharing more information)](/blog/_10/10_img-6.png)
...
<CodeBlock language="bash" content={`sudo apt install python3.12-venv`} />

Then.

<CodeBlock language="bash" content={`sudo python3 -m venv env`} />

- Activate project's Python environment.

<CodeBlock language="bash" content={`source env/bin/activate`} />

- Install project's dependencies.

First install **python3-pip**.

<CodeBlock language="bash" content={`sudo apt install python3-pip`} />

Then.

<CodeBlock language="bash" content={`pip install -r requirements.txt`} />

![(a post image sharing more information)](/blog/_10/10_img-7.png)
...
<CodeBlock language="bash" content={`echo "KEY=value" > .env`} />
...
![(a post image sharing more information)](/blog/_10/10_img-8.png)
...
![(a post image sharing more information)](/blog/_10/10_img-9.png)
...
<CodeBlock language="bash" content={`python3 manage.py runserver`} />
...
[read this previous article](/blog/categories/cloud-and-devops/complete-nodejs-back-end-deployment-on-aws-ec2). Even though
that article was a NodeJs/Express project deployment, it'll certainly be very helpful, as a result of it's more in-depth and
beginner-friendly approach.

> While we won't be opening the **8000** port to access the API directly, we'll need to open port
...
2. As a reverse proxy – Forwards client requests to backend servers (e.g., Node.js, Python).
...
![(a post image sharing more information)](/blog/_9/9_img-40.png)
...
<CodeBlock language="bash" content={`sudo nano /etc/nginx/nginx.conf`} />

![(a post image sharing more information)](/blog/_9/9_img-35.png)
...
<CodeBlock language="bash" content={`# include /etc/nginx/sites-enabled/*;`} />
...
![(a post image sharing more information)](/blog/_10/10_img-13.png)

- Visit the IP address - directly without the port. I.e. **http://13.60.96.165** in my case.

![(a post image sharing more information)](/blog/_10/10_img-14.png)
...
![(a post image sharing more information)](/blog/_10/10_img-35.png)
...
![(a post image sharing more information)](/blog/_10/10_img-36.png)

You should then see a success message that looks similar to the one below.

![(a post image sharing more information)](/blog/_10/10_img-37.png)
...
![(a post image sharing more information)](/blog/_10/10_img-12.png)
...
![(a post image sharing more information)](/blog/_10/10_img-15.png)
...
![(a post image sharing more information)](/blog/_10/10_img-17.png)

![(a post image sharing more information)](/blog/_10/10_img-18.png)

- Verify that the installation was successfully - run a **hello-world** image.

<CodeBlock language="bash" content={`sudo docker run hello-world`} />

![(a post image sharing more information)](/blog/_10/10_img-19.png)

- View list of available Docker images.

<CodeBlock language="bash" content={`sudo docker image ls`} />

![(a post image sharing more information)](/blog/_10/10_img-21.png)
...
<CodeBlock language="bash" content={`cat Dockerfile`} />

![(a post image sharing more information)](/blog/_10/10_img-22.png)
...
![(a post image sharing more information)](/blog/_10/10_img-23.png)

![(a post image sharing more information)](/blog/_10/10_img-24.png)
...
- **-d (detached mode)**: Runs the container in the background, so your terminal isn’t blocked by its output.
...
- **--name aws-ec2-django-deployment-project\_\_container**: Assigns a **custom name** to the container.
...
![(a post image sharing more information)](/blog/_10/10_img-25.png)
...
![(a post image sharing more information)](/blog/_10/10_img-28.png)
...
ExecStartPre=-/usr/bin/docker rm -f aws-ec2-django-deployment-project\_\_container

ExecStart=/usr/bin/docker run --name aws-ec2-django-deployment-project\_\_container --env-file /etc/secrets.env -p 8000:8000 aws-ec2-django-deployment-project

ExecStop=/usr/bin/docker stop aws-ec2-django-deployment-project**container
ExecStopPost=/usr/bin/docker rm -f aws-ec2-django-deployment-project**container

[Install]
WantedBy=multi-user.target
`} />

![(a post image sharing more information)](/blog/_10/10_img-29.png)
...
- **Description**: Describes the service — in this case, a Dockerized Python API server.

- **After=docker.service**: Ensures this service starts only after Docker is up and running.

- **Requires=docker.service**: Declares Docker as a hard dependency — if Docker stops, this service does too.
...
- **ExecStartPre**: Removes any existing container with the same name (aws-ec2-django-deployment-project\_\_container) to avoid naming conflicts.
...
<CodeBlock language="bash" content={`sudo rm -rf .env`} />

- Ensure that the deletion was successful.

<CodeBlock language="bash" content={`cat .env`} />

![(a post image sharing more information)](/blog/_10/10_img-30.png)
...
![(a post image sharing more information)](/blog/_10/10_img-33.png)

![(a post image sharing more information)](/blog/_10/10_img-34.png)
...
<CodeBlock language="bash" content={`git pull`} />
...
All of that and more will be covered later in a subsequent series, that will teach how to set up

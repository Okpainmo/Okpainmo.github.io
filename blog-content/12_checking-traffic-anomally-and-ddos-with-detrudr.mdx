...
<CodeBlock language="text" content={`zscore = (current_rate - baseline_mean) / baseline_stddev`} />

2. **Rate multiplier**

<CodeBlock language="text" content={`current_rate > baseline_mean * multiplier`} />
...
<CodeBlock language="bash" content={`iptables -I INPUT -s 198.51.100.10 -j DROP`} />

And when ban duration expires:

<CodeBlock language="bash" content={`iptables -D INPUT -s 198.51.100.10 -j DROP`} />
